...
$doReport = (!empty($_SERVER['argv'][2]) && $_SERVER['argv'][2] == 'doreport') ? true : false;
...
elseif(is_string($msgId)) {
	...
	if($msgId == "stateservices") {
		...
		if($doReport){
			$report_write = fopen($c_installDir."/wampConfReport_stateservices.txt","w");
			fwrite($report_write,$message['stateservices']);
			fclose($report_write);
			exit;
		}
	echo $message['stateservices'];
	...
	if(in_array('none',$dns)
	  ...
	  $complete_result = $message['dnscheckorder'];
		...
		if($doReport){
			$report_write = fopen($c_installDir."/wampConfReport_dnscheckorder.txt","w");
			fwrite($report_write,$message['dnscheckorder']);
			fclose($report_write);
			exit;
		}
	echo $message['dnscheckorder'];
	...
	elseif($msgId == "compilerversions") {
    	...
    	$apacheVersionTot[] = $oneApacheVersion;
			...
			foreach($result as $value) {
				if(strpos($value, 'built') !== false) {
					$output_1 = $value;
					preg_match('~^.*Lounge VC([0-9]{1,2}).*$~',$value, $matches);
		...
		foreach($phpVersionList as $onePhp) {
			...
			$command = 'start /b /wait '.$c_phpVersionDir.'/php'.$onePhpVersion.'/'.$wampConf['phpExeFile'].' -i';
			$output = exec($command, $result);
			$Compil = $Archi = false;
			foreach($result as $value){
				if(strpos($value, "Compiler") !== false) {
					$output_1 = $value;
					preg_match('~^.*MSVC([0-9]{1,2}).*$~',$value,$matches);
    ...
    foreach($phpCompiler as $key=>$value) {
    	...
    	foreach($apacheVersion as $apache) {
    		...
    		if($phpApacheDll[$key][$apache]) {
    			...
    			if($apacheVC[$apacheTot[1]] <= 11 && $phpVC[$key] >= 14) {
    ...
    if(($nb_v32 > 0 && $nb_v64 != 0) || ($nb_v64 > 0 && $nb_v32 !=0)) {
  	...
  	$message['inifiles'] = '';
		...
		if($doReport){
			$report_write = fopen($c_installDir."/wampConfReport_compilerversions.txt","w");
			fwrite($report_write,$message['compilerversions']);
			fclose($report_write);
			exit;
		}
	...
	elseif($msgId == "vhostconfig") {
		...
		if($doReport){
			$report_write = fopen($c_installDir."/wampConfReport_apachevhosts.txt","w");
			fwrite($report_write,$message['apachevhosts']);
			fclose($report_write);
			exit;
		}
		echo $message['apachevhosts'];
