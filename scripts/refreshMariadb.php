<?php
// 3.1.4 Service name check - config report

if(defined('SEE_PROCESS')) error_log("For information only : script refreshMariadb");
...
Type: item; Caption: "${w_testPortMysql}"; Action: run; FileName: "${c_phpExe}"; Parameters: "testPort.php 3306 ${c_mariadbService}";WorkingDir: "$c_installDir/scripts"; Flags: waituntilterminated; Glyph: 24
${MariaTestPortUsed}Type: item; Caption: "${w_testPortMariaUsed}${c_UsedMariaPort}"; Action: run; FileName: "${c_phpExe}"; Parameters: "testPort.php ${c_UsedMariaPort} ${c_mariadbService}";WorkingDir: "$c_installDir/scripts"; Flags: waituntilterminated; Glyph: 24
Type: item; Caption: "${w_AlternateMariaPort}"; Action: multi; Actions: UseAlternateMariaPort; Glyph: 24
Type:separator; Caption: "mariaDB Console prompt: ${mariadbConsolePromptUsed}"
Type: item; Caption: "${w_settings['mariadbUseConsolePrompt']}"; Glyph: 24; Action: multi; Actions: mariadbUseConsolePrompt
EOF;
$tpl = str_replace($myPattern,$myreplace,$tpl);
  ...
  if(preg_match("~^(.*mariadbServiceRemoveParams.*--remove )(wampmariadb[0-9]*)(.*\r?)$~mi",$mariaConfContents,$matches) > 0) {
  	if($matches[2] != $c_mariadbService)
		...
		if(!is_null($mariaConfContents) && $count > 0) {
			$fp = fopen($maConfile,'w');
			fwrite($fp,$mariaConfContents);
			fclose($fp);
		}
  ...
  include $maConfile;
	...
	if(strpos($maIniContents, "[".$c_mariadbService."]") === false) {
		...
		if(!is_null($maIniContents) && $count == 1) {
			$fp = fopen($maIniFile,'w');
			fwrite($fp,$maIniContents);
			fclose($fp);
