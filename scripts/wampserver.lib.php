<?php
//Change 3.1.6
// Apache Variable ${SRVROOT} taken into account.
//
//
...
function linkPhpDllToApacheBin($php_version) {
	...
	if(defined('SEE_PROCESS')) error_log("For information only : function linkPhpDllToApacheBin");
...
function CheckSymlink($php_version) {
	global $phpDllToCopy, $c_phpVersionDir, $c_apacheVersionDir, $wampConf, $phpConfFileForApache;
	if(defined('SEE_PROCESS')) error_log("For information only: function CheckSymlink");
	...
	foreach ($phpDllToCopy as $dll)
	{
		...
		if(is_file($target)) {
			...
			elseif(is_file($link)) {
				...
				$error .= "Symbolic link or file ".$link." does not exist\n";
			}
		}
	}
...
function switchPhpVersion($newPhpVersion)
{
	...
	if(defined('SEE_PROCESS')) error_log("For information only : function switchPhpVersion");
	...
	for($i = 0 ; $i < $nb_Server ; $i++) {
		...
		if(preg_match('/^
		  ...
		  [A-Za-z0-9]+ 		# letter or number in first place
			...
			$/x',$nameToCheck) == 0 && $wampConf['NotCheckVirtualHost'] == 'off') {
...
function ListVersions($used = false, $toutes = false) {
	...
	$phpUsed = '';
	$usedFound = $cliFound = false;
	foreach ($phpVersionList as $onePhp) {
		$onePhpVersion = str_ireplace('php','',$onePhp);
		$maydelete = true;
		if(!$usedFound && !$toutes && ($onePhpVersion == $c_phpVersion)) {
			$phpUsed = $onePhpVersion;
			$onePhpVersion .= 'USED';
			$usedFound = true;
			$maydelete = false;
		}
		if(!$cliFound && !$toutes && (($onePhpVersion == $c_phpCliVersion) || ($phpUsed == $c_phpCliVersion))) {
			$onePhpVersion .= 'CLI';
			$cliFound = true;
			$maydelete = false;
		}
...
//Check if a folder exixts then create it if not
