<?php
// - 3.1.7 Don't change vhosts port with apache version switching
//
//
//
//
...
if($doReport) {
	$wampReportTxt = '';
	$wampReportKeys = array('gen1','mysql','mariadb','gen2');
	$wampReport = array_fill_keys($wampReportKeys, '');
	$wampReport['gen1'] .= time()."\n------ Wampserver configuration report\n".date(DATE_RSS)."\n";
}

require ('config.inc.php');
require ('wampserver.lib.php');
if(defined('SEE_PROCESS')) error_log("For information only : script refresh");
	...
	if($wampConf['NotVerifyHosts'] == 'off') {
		...
		if($rewriteHost) {
			...
			if(flock($fp, LOCK_EX)) { // acquire an exclusive lock
			  ...
			  error_log('Unable to write to '.$c_hostsFile.' file');
			  if($doReport) {
			  	if(!$localIPv4)
...
if($wampConf['NotVerifyPATH'] == 'off') {
	...
	if($wampinstallfound || $phpinstallfound) {
		...
		if($doReport)
			$wampReport['gen2'] .= $message;
		$WarningsAtEnd = true;
		$WarningMenu .= 'Type: item; Caption: "Error '.$c_installDir.' or PHP in PATH"; Glyph: 19; Action: multi; Actions: warning_pathvar
';
...
// management of Change Services Names
if($wampConf['ItemServicesNames'] == 'on') {
	$changeServMenu = ';WAMPCHANGESERVICESNAMESMENU
';
	$changeServMenu .= <<< EOF
Type: separator; Caption: "${w_services}"
Type: item; Caption: "Apache:    ${c_apacheService}"; Action: multi; Actions: none
${SupportMySQL}Type: item; Caption: "MySQL:     ${c_mysqlService}"; Action: multi; Actions: none
${SupportMariaDB}Type: item; Caption: "MariaDB:   ${c_mariadbService}"; Action: multi; Actions: none
Type: item; Caption: "${w_changeServices}"; Action: multi; Actions: changeservicesnames; Glyph: 24
EOF;
	$tpl = str_replace(';WAMPCHANGESERVICESNAMESMENU',$changeServMenu,$tpl);

	$changeServMenu = ';WAMPCHANGESERVICESSTART
';
	$changeServMenu .= <<< EOF
[changeservicesnames]
...
Action: run; FileName: "${c_phpCli}"; Parameters: "switchServicesNames.php %SuffixServices%";WorkingDir: "${c_installDir}/scripts"; Flags: waituntilterminated
...
Action: exit;
EOF;
	$tpl = str_replace(';WAMPCHANGESERVICESSTART',$changeServMenu,$tpl);
}
...
if($wampConf['SupportMySQL'] == 'on' && $wampConf['SupportMariaDB'] == 'on') {
...
if($doReport) {
	...
	//Report files to add
	$report_files = array(
		'State of services' => 'wampConfReport_stateservices.txt',
		'Check DNS order' => 'wampConfReport_dnscheckorder.txt',
		'Compiler versions' => 'wampConfReport_compilerversions.txt',
		'VirtualHost examined by Apache' => 'wampConfReport_apachevhosts.txt',
		'Test port 80' => 'wampConfReport_testport80.txt',
		'Test port used by MySQL '.$c_mysqlPortUsed => 'wampConfReport_testport'.$c_mysqlPortUsed.'.txt',
		'Test port used by MariaDB '.$c_mariadbPortUsed => 'wampConfReport_testport'.$c_mariadbPortUsed.'.txt',
	);
	foreach($report_files as $key => $value) {
		if(file_exists($c_installDir."/".$value)){
			$wampReportTxt .= "\n-------- ".$key." --------\n";
			$wampReportTxt .= @file_get_contents($c_installDir."/".$value);
			$wampReportTxt .= "\n--------------------------------------------------";
			unlink($c_installDir."/".$value);
		}
	}
	...
	$report_write = fopen($c_installDir."/wampConfReport.txt","w");
	fwrite($report_write,clean_file_contents($wampReportTxt,array(1,0),true));
	fclose($report_write);
}
